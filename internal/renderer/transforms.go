...
func transformAdmonitions(html string) string {
    ...
    return re.ReplaceAllStringFunc(html, func(m string) string {
        ...
        if strings.TrimSpace(first) != "" { sb.WriteString(`<p>`+first+`</p>`) }
        sb.WriteString(rest)
    ...
    return reImg.ReplaceAllStringFunc(html, func(tag string) string {
        ...
        for i < len(tag) {
            if tag[i] == '"' {
                ...
                if k >= len(tag) || tag[k] == '>' || tag[k] == '/' || ( (tag[k]>='A'&&tag[k]<='Z') || (tag[k]>='a'&&tag[k]<='z') || tag[k]=='_' || tag[k]==':' ) { end = i; break }
            }
            i++
        }
        if end == -1 { return tag }
        ...
        src := ""; if m := reSrc.FindStringSubmatch(tag); len(m)>=2 { src = m[1] }
        title := ""; if m := reTitle.FindStringSubmatch(tag); len(m)>=2 { title = m[1] }
        var b strings.Builder
        b.WriteString("<img ")
        if src != "" { b.WriteString(`src="`+src+`"`) }
        if title != "" { b.WriteString(" "); b.WriteString(`title="`+title+`"`) }
        b.WriteString(" "); b.WriteString(`alt="`+val+`"`)
        b.WriteString(">")
        return b.String()
    })
}
...
func transformFootnotes(html string, numToLabel map[string]string) string {
    ...
    html = reSup.ReplaceAllStringFunc(html, func(m string) string {
        sub := reSup.FindStringSubmatch(m); if len(sub)<3 { return m }
        num := sub[1]; text := sub[2]
        label := num; if l, ok := numToLabel[num]; ok { label = l }
        counts[label]++
    ...
    html = reSection.ReplaceAllStringFunc(html, func(m string) string {
        inner := reSection.FindStringSubmatch(m); if len(inner)<2 { return m }
        list := inner[1]
        list = strings.ReplaceAll(list, "&#160;", " ")
        reId := regexp.MustCompile(`(?is)<li id=\"fn:([^\"]+)\"`)
        list = reId.ReplaceAllStringFunc(list, func(s string) string {
            x := reId.FindStringSubmatch(s); if len(x)<2 { return s }
            num := x[1]; label := num; if l, ok := numToLabel[num]; ok { label = l }
        ...
        list = reBack.ReplaceAllStringFunc(list, func(a string) string {
            x := reBack.FindStringSubmatch(a); if len(x)<2 { return a }
            num := x[1]; label := num; if l, ok := numToLabel[num]; ok { label = l }
            backCounts[label]++
            text := "↩"; if backCounts[label] > 1 { text = fmt.Sprintf("↩%d", backCounts[label]) }
...
func transformDefinitionLists(html string) string {
    ...
    buildDT := func(content string) string {
        ...
        id := slugify(plain)
        if id == "" { id = "term" }
        if cnt, ok := used[id]; ok { cnt++; used[id]=cnt; id = fmt.Sprintf("%s-%d", id, cnt) } else { used[id]=0 }
    ...
    return reDL.ReplaceAllStringFunc(html, func(block string) string {
        inner := reDL.FindStringSubmatch(block); if len(inner)<2 { return block }
        nodes := reNode.FindAllString(inner[1], -1); if len(nodes)==0 { return block }
        anyDDHasP := false
        for _, n := range nodes {
            if strings.HasPrefix(strings.ToLower(n), "<dd>") {
                ddInner := strings.TrimSpace(strings.TrimSuffix(strings.TrimPrefix(n, "<dd>"), "</dd>"))
                if strings.HasPrefix(strings.ToLower(ddInner), "<p>") { anyDDHasP = true; break }
            }
        }
        var sb strings.Builder
        sb.WriteString("<dl>")
        for i := 0; i < len(nodes); {
            ...
            if strings.HasPrefix(strings.ToLower(node), "<dt>") {
                ...
                for j < len(nodes) && strings.HasPrefix(strings.ToLower(nodes[j]), "<dt>") {
                    m := reDTInner.FindStringSubmatch(nodes[j]); if len(m)>=2 { parts = append(parts, m[1]) }
                    j++
                }
                if len(parts) <= 1 {
                    ...
                    for k := 0; k < len(parts); k++ {
                        ...
                        if endsBS { t = strings.TrimSuffix(t, "\\") }
                        disp.WriteString(htmlEscape(t))
                        if k < len(parts)-1 { if endsBS { disp.WriteString("<br>") } else { disp.WriteString(" ") } }
                    }
                ...
                if !strings.Contains(inner, "<") {
                    ...
                    if len(parts) >= 2 {
                        ...
                        buildNested = func(items []string) string {
                            ...
                            if len(items)==1 { return htmlEscape(strings.TrimSpace(items[0])) }
                            var nb strings.Builder
                            nb.WriteString("<dl>")
                            nb.WriteString(buildDT(htmlEscape(strings.TrimSpace(items[0]))))
                            nb.WriteString("<dd>")
                            if len(items)==2 { nb.WriteString(htmlEscape(strings.TrimSpace(items[1]))) } else { nb.WriteString(buildNested(items[1:])) }
