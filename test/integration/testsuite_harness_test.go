...
func TestSelectedVendoredSuites(t *testing.T) {
	...
	for _, rel := range cases {
		t.Run(rel, func(t *testing.T) {
			...
			pipe := p.NewPipeline()
			pipe.Add(pindex.NewIndexPreprocessor())
			...
			err = filepath.WalkDir(expDir, func(path string, d fs.DirEntry, err error) error {
				...
				require.NoError(t, err)
				exp := string(expBytes)
				if strings.Contains(exp, "playground") || strings.Contains(exp, "editable") {
					return nil
				}
				...
				i := strings.Index(lower, "<main>")
				j := strings.Index(lower, "</main>")
				var got string
				if i >= 0 && j > i {
